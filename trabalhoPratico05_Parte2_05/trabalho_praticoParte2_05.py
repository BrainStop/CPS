...
#    TODO pnzr1C1=pnzr1*recetorNormado???
#    pnzr2C2=pnzr1*recetorNormado???
    signalQPSK=pnzr1C1+pnzr2C2
    return signalQPSK
    
###testes a QPSK
qpsk=toQPSK(Stream,even,Eb)
toString("qpsk=",qpsk)
plt.figure()
...
#daqui para baixo é trabalho anterior
    
#exercicio 2
lamba=0
PNRZStream=toPNRZ(Stream, even, Amp)

def decodePNRZ(przStream, even,lambaDecisionValue):
    pnrz_split = np.reshape(przStream, (-1, even))
    media = np.mean(pnrz_split, axis=1)
